...
def filter_config_diff(previous_system, current_system):
    diffs = config_difference(previous_system, current_system).split("\n")
    for d in diffs:
        if (
            d.startswith("+")
            and d.split("+")[-1] + "\n" in previous_system.config_text()
        ) or (
            d.startswith("-")
            and d.split("-")[-1].split("\n")[0] in current_system.config_text()
        ):
            diffs.remove(d)
    return diffs


class ConfigDifferenceDialog(UIBuilderWidget):
    def __init__(
        self,
        ...
        for d in diffs:
            if d.startswith("+") and d.split("+")[-1] in current_system.config_text():
                new_store.append([d])
