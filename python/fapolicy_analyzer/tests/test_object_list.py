import context  # noqa: F401
import gi
import pytest
import re

gi.require_version("Gtk", "3.0")
from gi.repository import Gtk
from unittest.mock import MagicMock
from ui.configs import Colors
from ui.object_list import ObjectList
from ui.strings import FILE_LABEL, FILES_LABEL

_objects = [
    {"status": "ST", "mode": "R", "access": "A", "path": "/tmp/foo"},
    {"status": "AT", "mode": "W", "access": "A", "path": "/tmp/baz"},
    {"status": "U", "mode": "X", "access": "D", "path": "/tmp/bar"},
]


@pytest.fixture
def widget():
    return ObjectList()


def test_creates_widget(widget):
    assert type(widget.get_ref()) is Gtk.Box


def test_loads_store(widget):
    def strip_markup(markup):
        return re.search(r"<u>([A-Z]*)</u>", markup).group(1)

    widget.load_store(_objects)
    view = widget.get_object("treeView")
    assert [t["status"] for t in _objects] == [
        strip_markup(x[0]) for x in view.get_model()
    ]
    assert [t["mode"] for t in _objects] == [
        strip_markup(x[1]) for x in view.get_model()
    ]
    assert [t["access"] for t in _objects] == [
        strip_markup(x[2]) for x in view.get_model()
    ]
    assert [t["path"] for t in _objects] == [x[3] for x in view.get_model()]


def test_status_markup(widget):
    view = widget.get_object("treeView")

    # System trust
    widget.load_store([{"status": "ST"}])
    assert view.get_model()[0][0] == "<b><u>ST</u></b>/AT/U"
    # Ancillary trust
    widget.load_store([{"status": "AT"}])
    assert view.get_model()[0][0] == "ST/<b><u>AT</u></b>/U"
    # Untrusted
    widget.load_store([{"status": "U"}])
    assert view.get_model()[0][0] == "ST/AT/<b><u>U</u></b>"
    # Bad data
    widget.load_store([{"status": "foo"}])
    assert view.get_model()[0][0] == "ST/AT/U"
    # Empty data
    widget.load_store([{}])
    assert view.get_model()[0][0] == "ST/AT/U"
    # Lowercase
    widget.load_store([{"status": "st"}])
    assert view.get_model()[0][0] == "<b><u>ST</u></b>/AT/U"


def test_mode_markup(widget):
    view = widget.get_object("treeView")

    # Read
    widget.load_store([{"mode": "R"}])
    assert view.get_model()[0][1] == "<b><u>R</u></b>WX"
    # Write
    widget.load_store([{"mode": "W"}])
    assert view.get_model()[0][1] == "R<b><u>W</u></b>X"
    # Execute
    widget.load_store([{"mode": "X"}])
    assert view.get_model()[0][1] == "RW<b><u>X</u></b>"
    # Read/Write
    widget.load_store([{"mode": "RW"}])
    assert view.get_model()[0][1] == "<b><u>R</u></b><b><u>W</u></b>X"
    # Read/Execute
    widget.load_store([{"mode": "RX"}])
    assert view.get_model()[0][1] == "<b><u>R</u></b>W<b><u>X</u></b>"
    # Write/Execute
    widget.load_store([{"mode": "WX"}])
    assert view.get_model()[0][1] == "R<b><u>W</u></b><b><u>X</u></b>"
    # Full Access
    widget.load_store([{"mode": "RWX"}])
    assert view.get_model()[0][1] == "<b><u>R</u></b><b><u>W</u></b><b><u>X</u></b>"
    # Bad data
    widget.load_store([{"mode": "foo"}])
    assert view.get_model()[0][1] == "RWX"
    # Empty data
    widget.load_store([{}])
    assert view.get_model()[0][1] == "RWX"
    # Lowercase
    widget.load_store([{"mode": "r"}])
    assert view.get_model()[0][1] == "<b><u>R</u></b>WX"


def test_access_markup(widget):
    view = widget.get_object("treeView")

    # Allowed
    widget.load_store([{"access": "A"}])
    assert view.get_model()[0][2] == "<b><u>A</u></b>/D"
    # Denied
    widget.load_store([{"access": "D"}])
    assert view.get_model()[0][2] == "A/<b><u>D</u></b>"
    # Bad data
    widget.load_store([{"access": "foo"}])
    assert view.get_model()[0][2] == "A/D"
    # Empty data
    widget.load_store([{}])
    assert view.get_model()[0][2] == "A/D"
    # Lowercase
    widget.load_store([{"access": "a"}])
    assert view.get_model()[0][2] == "<b><u>A</u></b>/D"


def test_path_color(widget):
    view = widget.get_object("treeView")

    # Denied
    widget.load_store([{"access": "D", "mode": "RWX"}])
    assert view.get_model()[0][5] == Colors.LIGHT_RED

    # Full Access
    widget.load_store([{"access": "A", "mode": "RWX"}])
    assert view.get_model()[0][5] == Colors.LIGHT_GREEN
    # Partical Access
    widget.load_store([{"access": "A", "mode": "R"}])
    assert view.get_model()[0][5] == Colors.LIGHT_YELLOW
    # Bad data
    widget.load_store([{"access": "foo"}])
    assert view.get_model()[0][5] == Colors.LIGHT_RED
    # Empty data
    widget.load_store([{}])
    assert view.get_model()[0][5] == Colors.LIGHT_RED
    # Lowercase
    widget.load_store([{"access": "a", "mode": "rwx"}])
    assert view.get_model()[0][5] == Colors.LIGHT_GREEN


def test_update_tree_count(widget):
    widget.load_store([{}])
    label = widget.get_object("treeCount")
    assert label.get_text() == f"1 {FILE_LABEL}"

    widget.load_store([{}, {}])
    label = widget.get_object("treeCount")
    assert label.get_text() == f"2 {FILES_LABEL}"


def test_fires_object_selection_changed_event(widget):
    mockHandler = MagicMock()
    widget.object_selection_changed += mockHandler
    mockData = {"path": "foo"}
    widget.load_store([mockData])
    view = widget.get_object("treeView")
    view.get_selection().select_path(Gtk.TreePath.new_first())
    mockHandler.assert_called_with(mockData)
